...
What does it mean? Version control systems store commits, snapshots of files and
how they changed over time. In most, you "check out" some version of the code,
edit some files, then run commands to push those edits back in.

In jj, you instead are always (conceptually) _directly editing a current
...
working on something else, your work is already saved. (This replaces
`git 
stash`.)
...
changing the description of the newest one. (This replaces
`git commit --amend`.)
...
Instead the commands succeed and create conflicting commits, and you resolve
conflicts by directly editing those commits using the same core commands as
elsewhere.
